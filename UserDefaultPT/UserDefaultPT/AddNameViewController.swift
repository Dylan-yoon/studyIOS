...
class AddNameViewController: UIViewController {
    ...
    @IBOutlet weak var identityAge: UILabel!
    
    var nameage = [NameAge]()
//    userDefault 사용 전 사용 할 배열
//    var recodeNameage = [NameAge]()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
      
        //화면 켰을 때 커서가 텍스트 필드에 위치하도록 해주는메서드
        nameLabel.becomeFirstResponder()
        
        
        //didchange,, 텍스트 필드가 실시간 업데이트
        self.nameLabel.addTarget(self, action: #selector(self.textFieldDidChange(_:)), for: .editingChanged)
        self.ageLabel.addTarget(self, action: #selector(self.textFieldDidChange(_:)), for: .editingChanged)
    }
    
    @IBAction func addBtn(_ sender: UIButton) {
        guard let name = self.nameLabel.text else { return }
        guard let age = self.ageLabel.text else { return }
        
        
        let total = NameAge(name: name, age: age)
        self.nameage.append(total) 
    ...
    @objc func textFieldDidChange(_ sender: Any?) {
        self.identityName.text = self.nameLabel.text
        self.identityAge.text = self.ageLabel.text
    }
